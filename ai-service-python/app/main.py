...
def parse_knowledge(request: dict[str, Any]) -> dict[str, Any]:
    ...
    source_type = infer_source_type(source_type, file_name)
    source_id = request.get("sourceId")
    graph_category_id = request.get("graphCategoryId")
    blocks = build_blocks(raw_text, source_type, source_id, graph_category_id)
    return {
        "sourceId": source_id,
        "blocks": blocks,
        "degraded": False,
...
def infer_source_type(source_type: str, file_name: str) -> str:
    ...
    if source_type == "text" and suffix in {"md", "markdown"}:
        return "md"
    if source_type == "text" and suffix in {"sql", "ddl"}:
        return "sql"
    return source_type
...
def build_blocks(raw_text: str, source_type: str, source_id: Any, graph_category_id: Any) -> list[dict[str, Any]]:
    ...
    if source_type == "json":
        try:
            ...
            return [block_envelope(source_id, graph_category_id, "json", text, text, 1, None, None, {"parser": "json"})]
        except json.JSONDecodeError:
            pass
    if source_type == "csv":
        return csv_blocks(raw_text, source_id, graph_category_id)
    if source_type == "md":
        return markdown_blocks(raw_text, source_id, graph_category_id)
    if source_type == "sql":
        return sql_blocks(raw_text, source_id, graph_category_id)
    ...
    for index, line in enumerate(lines, start=1):
        ...
        blocks.append(block_envelope(source_id, graph_category_id, block_type, line, normalize_text(line), index, None, None, {"parser": "text"}))
    return blocks
...
def check_llm_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if local_strict:
        ...
        result["ok"] = bool(result["modelsOk"] and result["modelFound"] and result["chatOk"])
    else:
